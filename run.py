...
def setup_env_file(install_path: Optional[str]) -> None:
    ...
    """
    env_file_path = os.path.join(script_dir, 'server', '.env')
    ...
    if install_path:
        ...
        logger.warning("未能检测到BetterGI安装路径。请手动在/server/.env中填写安装BetterGI的路径")
...
def setup_python_env() -> bool:
    ...
    # 定义项目根目录和虚拟环境目录
    project_root = os.path.join(script_dir, 'server')
...
def start_backend_server() -> Optional[subprocess.Popen]:
    ...
    venv_dir = os.path.join(script_dir, 'server', '.venv')
...
def main() -> int:
    ...
    try:
        ...
        if frontend_server:
            server_processes.append(frontend_server)
        else:
            logger.error("前端服务器启动失败")
            cleanup_resources()  # 确保已启动的后端服务被终止
            return 1
        ...
        logger.info("BetterGI日志分析工具已成功启动，你可以通过 http://localhost:3000 进行访问。")
