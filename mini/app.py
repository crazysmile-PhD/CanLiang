...
# 创建全局DataFrame对象，用于存储物品信息
items_df = pd.DataFrame(columns=['物品名称', '时间', '日期'])
...
def parse_log(log_content):
    ...
    for match in matches:
        ...
        if '交互或拾取' in details:
            ...
            # 记录物品的时间戳
            if item not in item_timestamps:
    ...
    return {
        ...
        'item_timestamps': item_timestamps,  # 新增：返回物品时间戳信息
...
def get_log_list():
    ...
    同时将解析到的物品信息添加到全局DataFrame中。
    
    Returns:
        list: 过滤后的日志文件名列表
    """
    global items_df
    # 重置DataFrame，避免重复添加数据
    items_df = pd.DataFrame(columns=['物品名称', '时间', '日期'])
    ...
    for file in log_files:
        ...
        if items:
            ...
            # 提取日期信息，格式化为YYYY-MM-DD
            try:
                # 假设file格式为YYYYMMDD或类似格式
                date_str = file
                if len(date_str) >= 8:  # 确保有足够的字符来解析日期
            ...
            for item_name, count in items.items():
                # 获取该物品的时间戳列表
                timestamps = result.get('item_timestamps', {}).get(item_name, [])
                
                # 如果有时间戳记录，则使用实际时间戳
                if timestamps and len(timestamps) >= count:
                    for i in range(count):
                        time_info = timestamps[i]
                        new_row = pd.DataFrame([{'物品名称': item_name, '时间': time_info, '日期': formatted_date}])
                        items_df = pd.concat([items_df, new_row], ignore_index=True)
                else:
                    # 如果没有足够的时间戳记录，则使用空字符串
