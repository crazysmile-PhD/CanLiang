...
import { useDateList } from "@/hooks/use-date-list"
import { useInventoryData } from "@/hooks/use-inventory-data"
import { useItemTrend } from "@/hooks/use-item-trend"
import { useStatTrend } from "@/hooks/use-stat-trend"
  ...
  export default function InventoryPage() {
    ...
    const { statModal, handleStatClick, closeStatModal } = useStatTrend()
    const [searchTerm, setSearchTerm] = useState("")
    const [isChangingDate, setIsChangingDate] = useState(false)
    const [animationKey, setAnimationKey] = useState(0)
    const [hoverIndex, setHoverIndex] = useState<number | null>(null)
    const loading = dateListLoading || dataLoading
    const error = dateListError || dataError
