...
import { CategoryTotal } from "@/types/inventory"
...
export const getCategoryTotals = (categories: Record<string, Record<string, number>>) => {
...
export const getTopItems = (items: Record<string, number>, limit = 15) => {
  return Object.entries(items)
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit)
}

export const filterAndSortItems = (items: Record<string, number>, searchTerm: string) => {
  return Object.entries(items)
    .filter(([name]) => name.toLowerCase().includes(searchTerm.toLowerCase()))
    .sort((a, b) => b[1] - a[1])
}

export const getTotalItemCount = (items: Record<string, number>) => {
  return Object.values(items).reduce((sum, count) => sum + count, 0)
}

export const getUniqueItemCount = (items: Record<string, number>) => {
