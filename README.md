...
- 如果需要在浏览器端进行实时画面预览，更合适的方案是改用开放编码的流媒体技术：
  - **WebRTC (H.264/VP9)**：透过 WebRTC 网页端可以直接访问 H.264 或 VP9 编码的视频流，同时具备低延迟、双向通道等特性，适合即时交互场景。
  - **LL-HLS (MSE)**：利用低延迟 HLS 并结合 Media Source Extensions，可以维持较低延迟并兼顾播放器兼容性，适用于以观众为主的实时预览。
- 两种方案都可在采集端通过软件转码为 H.264/VP9 或其他浏览器支持的编码，再推流到网页端播放器，确保在浏览器环境下能够稳定观看。
