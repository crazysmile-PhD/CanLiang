...
统计汇整器会自动检测日志版本。若识别为旧格式，将尝试转换为新的字段；转换失败的日志会被标记为 `legacy` 并在统计时跳过。

对于批量升级旧日志，可使用 `upgrade_logs.py` 脚本。该脚本在转换前会创建 `.bak` 备份文件。如需回退，将备份文件还原即可恢复原始日志。
