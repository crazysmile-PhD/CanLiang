...
def cleanup_resources() -> None:
    ...
    # 在打包环境中，可能需要特殊处理线程终止
    if is_packaged:
        logger.info("正在清理打包环境资源...")
        # 由于在打包环境中使用了线程而不是子进程，这里不需要特别处理
        # 线程会随着主程序的退出而终止
        # 但我们仍然需要处理可能存在的伪进程对象
            ...
            if process:
                try:
                    if hasattr(process, 'terminate'):
                        process.terminate()
                    logger.info("已终止后端服务线程")
                except Exception as e:
                    logger.error(f"终止服务时出错: {e}")
    else:
        # 非打包环境，使用原来的方式终止进程
        for process in server_processes:
...
def start_backend_server() -> Optional[subprocess.Popen]:
    ...
    # 如果app.py不存在，可能是在打包环境中，尝试直接运行app模块
    if is_packaged or not os.path.exists(app_path):
        logger.info("检测到打包环境，将直接运行app模块...")
        try:
            # 在打包环境中直接运行Python解释器
            server = None
            if system == "Windows":
                ...
                # 使用线程运行Flask应用
                import threading
                server_thread = threading.Thread(target=backend_app.app.run, kwargs={'debug': False, 'host': '0.0.0.0', 'port': 5000})
                ...
                server = PseudoProcess()
                logger.info("后端服务器已在线程中启动")
            else:
                logger.error(f"打包环境下暂不支持的操作系统: {system}")
                return None
                
            return server
        except Exception as e:
            logger.error(f"在打包环境中启动后端服务器时出错: {e}")
            return None
    else:
        # 非打包环境，使用原来的方式启动
